...
This script consolidates the final v3 stacking and calibration stage from the
available project artifacts. It validates the raw data, rebuilds the final
stacker features that are cheap to recompute, consumes cached OOF/test
predictions from the original experiments, and writes a reproduced submission.

Important scope note:
    The selected v3 solution depends on cached transformer fine-tune,
    embedding, and earlier stack predictions. Their source scripts are retained
    in src/ and gm/, but the exact v3 final file was generated by stacking those
    cached artifacts rather than by training every base learner inside one
    monolithic script. This file reproduces the selected final CSV from raw CSV
    files plus those saved artifacts.
...
Optional:
    ...
    python reproduce_submission_v3_tv_q.py --write-all-strategies
"""
...
def compare_with_expected(output: pd.DataFrame, expected_path: Path) -> None:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--expected",
        default="outputs/submission_v3_tv_q.csv",
        help="Existing selected final CSV used for exact comparison.",
    )
    parser.add_argument(
...
def reproduce_from_saved_v3_artifact(
    ...
    print(f"  TV marginal calibration target: {np.round(tv_dist, 4).tolist()}")
    print(f"  wrote: {output_path}")
    print("  predicted label distribution:")
    print(f"  {submission['Label'].value_counts(normalize=True).sort_index().round(4).to_dict()}")
...
def main() -> None:
    ...
    submission.to_csv(output_path, index=False)
    print(f"\nWrote reproduced final submission: {output_path}")
    print(f"TV marginal calibration target: {np.round(tv_dist, 4).tolist()}")
    print("Predicted label distribution:")
    print(submission["Label"].value_counts(normalize=True).sort_index().round(4).to_dict())
